...
def My_marks(request):
    if request.session.has_key('Student_email'):
        data_obj = request.session['Student_email']
        data_obj = Subject.objects.all()
        return render(request, 'My_marks.html', {'data': data_obj})
    else:
        return render(request, 'Home.html')
...
def View_Tea_Ass(request):
    if request.session.has_key('Teacher_email'):
        a = request.session['Teacher_email']
        data_obj = Teacher_Assignment.objects.all()
        return render(request, 'Submit_Assignment.html', {'data': data_obj})
    else:
        return render(request, 'Home.html')

def download_Assignment_T(request):
    user_id = request.GET['ids']
    obj = Teacher_Assignment.objects.get(id=user_id)
    file_name = obj.File_Name
    file_path = settings.MEDIA_ROOT + '/' + file_name
    file_wrapper = FileWrapper(open(file_path, 'rb'))
    file_mimetype = mimetypes.guess_type(file_path)
